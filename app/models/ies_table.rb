class IesTable < ApplicationRecord
    ...
    validates :nome, presence: true {message: "ERRO: Nome da instituição não infomado."},
                   uniqueness: true {message: "ERRO: Instituição de ensino já cadastrada."}
    ...
    validates :cnpj, uniqueness: true {message: "ERRO: CNPJ já cadastrado."},
                   numericality: true, length: {maximum: 14}
        #============ não utilizado, substituído pelo método acima ============
        # #Certifica que o elemento seja único
